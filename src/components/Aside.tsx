        ...
        <aside className={`max-sm:absolute ${isOpen} max-sm:w-2/3 max-sm:transition-[left] flex flex-col gap-8 h-full p-4 overflow-hidden z-40 bg-primary-bg sm:relative duration-100 transition-[padding,width] ${isMenuOpen
            ...
            <div className="flex flex-col gap-8 h-full">
                <header className={`flex max-sm:justify-between items-center gap-1 py-1 h-fit`}>
                    ...
                    <h2 className={`h-fit text-primary text-nowrap overflow-hidden transition-[width,display] transition-discrete duration-100 ${isMenuOpen ? 'sm:block sm:w-full' : 'sm:starting:w-full sm:w-0 sm:hidden'}`}>
                        Pandora Box
                    </h2>
